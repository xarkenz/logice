Apper.Gallery = class {

    #ID;
    #app;
    #frame;
    #element;
    #title;

    get ID() { return this.#ID; }
    get app() { return this.#app; }
    get element() { return this.#element; }
    get title() { return this.#title.textContent; }
    set title(text) { return this.#title.textContent = text; }

    constructor(app, urls = null) {
        this.#ID = Apper.Menu.nextID++;
        this.#app = app;

        this.#frame = document.createElement("div");
        this.#frame.className = "apper-gallery";
        this.#frame.style.visibility = "hidden";
        this.#frame.addEventListener("transitionend", event => {
            if (!this.#frame.classList.contains("apper-shown"))
                this.#frame.style.visibility = "hidden";
        }, { passive: true });
        this.#app.element.appendChild(this.#frame);

        this.#element = document.createElement("div");
        this.#frame.appendChild(this.#element);

        if (urls) urls.forEach(url => {
            let img = document.createElement("img");
            img.src = url;
            this.#element.appendChild(img);
        });
    }

    show() {
        this.#frame.style.visibility = "visible";
        this.#frame.classList.add("apper-shown");
        return this;
    }

    hide() {
        this.#frame.classList.remove("apper-shown");
        return this;
    }

    add(url) {
        let img = document.createElement("img");
        img.src = url;
        this.#element.appendChild(img);
        return this;
    }

    addSeparator() {
        let separator = document.createElement("span");
        separator.className = "apper-separator";
        this.#element.appendChild(separator);
        return this;
    }

};

/*Apper.prototype.addGallery = (urls = null) => {
  const gallery = new Apper.Gallery(this, urls);
  return this.#menus[gallery.ID] = gallery;
};*/


class Logice extends Apper {

    get dragging() { return this.drag != null; }
    get grid() { return { x: this.widget.gridWidth.value, y: this.widget.gridHeight.value }; }

    constructor() {
        super();

        this.HOST_URL = "https://raw.githubusercontent.com/xarkenz/logice/main/"
        this.CURVE_SAMPLE_INTERVAL = 0.01;
        this.REVERT_PROXIMITY = 8;
        this.RESIZE_PROXIMITY = 12;
        this.STRAIGHTEN_RADIUS = 20;
        this.WHEEL_ZOOM_COEFF = 0.0005;
        this.KEY_ZOOM_AMOUNT = 0.1;
        this.MAX_ITERATIONS = 1000;

        this.enableToolbar()
            .addTool(Logice.Tool.INTERACT, true)
            .addTool(Logice.Tool.SELECT)
            .addTool(Logice.Tool.MOVE)
            .addTool(Logice.Tool.DRAW)
            .addTool(Logice.Tool.ERASE)
            .addTool(Logice.Tool.STYLE)
            .addTool(Logice.Tool.GRID)
            .addSpacer()
            .addTool(Logice.Tool.CAPTURE)
            .addTool(Logice.Tool.DATA)
            .addTool(Logice.Tool.LOAD)
            .addTool(Logice.Tool.HELP);

        this.widget = {};
        this.colorOptions = [
            "transparent", Logice.Color.LIGHT, Logice.Color.MEDIUM, Logice.Color.DARK,
            Logice.Color.RED, Logice.Color.ORANGE, Logice.Color.YELLOW, Logice.Color.GREEN,
            Logice.Color.CYAN, Logice.Color.BLUE, Logice.Color.PURPLE, Logice.Color.PINK,
        ];

        this.drawMenu = this.addMenu("Add a Component")
            .addSeparator()
            .add(new Apper.Menu.ButtonList("Component List", Object.keys(Logice.componentType), Object.values(Logice.componentType).map(ctype => ctype.display))
                .onChange(key => {
                    this.heldComponent = new Logice.Component(Logice.componentType[key], 0, 0, this.heldComponent?.direction ?? Logice.Direction.E);
                    this.update();
                }));
        this.styleMenu = this.addMenu("Element Style")
            .addSeparator()
            .add(this.widget.fillColor = new Apper.Menu.HSpread("vertedge-fill-color", "Fill color:", this.colorOptions.map(color => Logice.fillIcon(color)), "icons/color.svg")
                .onChange(value => {
                    this.selection.forEach(element => {
                        if (element instanceof Logice.Vertex) element.fill = this.colorOptions[value];
                    });
                    this.update();
                }))
            .add(this.widget.lineColor = new Apper.Menu.HSpread("vertedge-line-color", "Line color:", this.colorOptions.map(color => Logice.strokeIcon(color)), "icons/stroke.svg")
                .onChange(value => {
                    this.selection.forEach(element => {
                        element.stroke = this.colorOptions[value];
                    });
                    this.update();
                }))
            .addSeparator()
            .add(this.widget.vertexShape = new Apper.Menu.HSpread("vertedge-vertex-shape", "Shape:", ["icons/shape-circle.svg", "icons/shape-square.svg", "icons/shape-diamond.svg"], "icons/shape.svg")
                .onChange(value => {
                    this.selection.forEach(element => {
                        if (element instanceof Logice.Vertex) element.shape = value;
                    });
                    this.update();
                }))
            .add(this.widget.vertexRadius = new Apper.Menu.NumberInput("vertedge-vertex-radius", "Radius:", "icons/radius.svg").setMin(0)
                .onChange(value => {
                    this.selection.forEach(element => {
                        if (element instanceof Logice.Vertex) element.r = this.widget.vertexRadius.value;
                    });
                    this.update();
                }))
            .add(this.widget.lineWidth = new Apper.Menu.NumberInput("vertedge-line-width", "Line width:", "icons/line-width.svg").setMin(0)
                .onChange(value => {
                    this.selection.forEach(element => {
                        element.lineWidth = this.widget.lineWidth.value;
                    });
                    this.update();
                }))
            .add(this.widget.lineDash = new Apper.Menu.Checkbox("vertedge-line-dash", "Dashed line")
                .onChange(value => {
                    this.selection.forEach(element => {
                        element.lineDash = value ? [2, 2] : [];
                    });
                    this.update();
                }));

        this.gridMenu = this.addMenu("Grid Settings")
            .addSeparator()
            .add(this.widget.enableAxes = new Apper.Menu.Checkbox("vertedge-enable-axes", "Show axes")
                .onChange(value => this.update()))
            .add(this.widget.enableGrid = new Apper.Menu.Checkbox("vertedge-enable-grid", "Enable grid", true)
                .onChange(value => this.update()))
            .add(this.widget.gridWidth = new Apper.Menu.NumberInput("vertedge-grid-width", "Horizontal spacing:", "icons/width.svg", 12).setMin(1)
                .onChange(value => this.update()))
            .add(this.widget.gridHeight = new Apper.Menu.NumberInput("vertedge-grid-height", "Vertical spacing:", "icons/height.svg", 12).setMin(1)
                .onChange(value => this.update()));

        this.captureMenu = this.addMenu("Capture Image")
            .addSeparator()
            .add(this.widget.capturePrompt = new Apper.Menu.Paragraph("To capture a graph image, try adding some elements to the graph."))
            .add(this.widget.captureInstructions = new Apper.Menu.Paragraph("Resize the box to crop the resulting image. To move the view, drag from outside the box.").hide())
            .add(this.widget.captureSelectionOnly = new Apper.Menu.Checkbox("vertedge-capture-selection-only", "Capture selection only").hide()
                .onChange(value => this.update()))
            .add(this.widget.captureFitGraph = new Apper.Menu.Button("Fit Entire Graph").hide()
                .onClick(() => this.fitCaptureTo(this.vertices.concat(this.edges))))
            .addSeparator()
            .add(this.widget.captureEmptyPreview = new Apper.Menu.Paragraph("An image preview will appear here."))
            .add(this.widget.capturePreview = new Apper.Menu.CanvasImage("Preview:").hide())
            .add(this.widget.captureDownload = new Apper.Menu.Button("<img src='icons/download.svg'/> Download as PNG").hide());

        this.dataMenu = this.addMenu("Raw Graph Data")
            .addSeparator()
            .add(this.widget.dataEditor = new Apper.Menu.TextEditor("vertedge-data-editor", "Raw JSON data...")
                .onChange(text => {
                    if (!text) this.loadFromData({});
                    else try {
                        this.loadFromData(JSON.parse(text));
                        this.widget.dataEditor.valid = true;
                    } catch (err) {
                        if (!(err instanceof SyntaxError)) console.error(err);
                        this.widget.dataEditor.valid = false;
                    }
                    this.update();
                }));

        this.exampleURLs = [
            "examples/complete-4",
            "examples/complete-5",
            "examples/complete-3-3"
        ];
        this.exampleTitles = [
            "Complete Graph: n = 4",
            "Complete Graph: n = 5",
            "Complete Bipartite Graph: m = 3, n = 3"
        ];

        this.loadMenu = this.addMenu("Load a Graph")
            .addSeparator()
            .add(new Apper.Menu.ButtonList("Example Graphs:", this.exampleURLs, this.exampleTitles)
                .onChange(url => {
                    this.update();
                    this.load(url);
                    this.tool = this.toolbar.defaultTool;
                }));

        this.helpModal = this.addModal("Vertedge Help")
            .addSeparator()
            .add(new Apper.Menu.Paragraph("Hi! I'm a former AMR student, so I hope you'll understand that this is still a work in progress."))
            .add(new Apper.Menu.Paragraph("If you still need help beyond this page, you can email me at <a href='mailto:seanedwardsclarke@gmail.com' target='_blank' rel='noopener noreferrer'>seanedwardsclarke@gmail.com</a>."))
            .add(new Apper.Menu.Paragraph("Please note that the only way to save graphs right now is to save the data using the <b>View Data</b> menu, or an image using the <b>Capture</b> tool. See the section below for more information on how to use these."))
            .add(new Apper.Menu.Paragraph("Found a bug or issue? You can report it on <a href='https://github.com/xarkenz/vertedge/issues' target='_blank' rel='noopener noreferrer'>the GitHub page</a> or in an email to the address above. Thanks for your help!"))
            .addSeparator()
            .add(new Apper.Menu.Paragraph("- You can center the view on the graph by pressing the <b>Period</b> key.<br>- Holding <b>Shift</b> while clicking on objects adds them to the current selection.<br>- While dragging in <b>Draw</b> mode to draw an edge, holding the <b>Control</b> key allows you to create a loop."))
            .addSeparator()
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/select.svg'/><b>Select</b> (v)<br><br>This is the default tool. Click on vertices and edges to select them, and drag them to move them around. If you drag an edge, it will become a curve which you can adjust to your needs. To select multiple elements at once, drag from empty space to select a region or hold <b>Shift</b> when clicking on elements."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/move.svg'/><b>Move</b> (m)<br><br>This tool allows you to move the entire view by dragging. This can be especially helpful if you find yourself running out of room on the screen."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/draw.svg'/><b>Draw</b> (d)<br><br>This tool allows you to create new edges and vertices simply by clicking and dragging. You can even split edges by clicking on them."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/erase.svg'/><b>Erase</b> (x)<br><br>Use this tool to remove vertices and edges from the graph. You can also use the <b>Delete</b> key to remove the selection in any mode."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/style.svg'/><b>Style</b> (s)<br><br>This tool allows you to modify the appearance of selected elements through a menu."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/grid.svg'/><b>Grid</b> (g)<br><br>The grid can be enabled/disabled and tweaked through this menu if you want to make your graph more orderly."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/capture.svg'/><b>Capture</b><br><br>This tool allows you to download a PNG image of a region of your graph. The background is transparent, so paste wherever you need!"))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/data.svg'/><b>View Data</b><br><br>This menu contains the raw JSON data for the graph. If you want to save your graph, this would be the way to do it: copy the raw data and paste it somewhere else for safekeeping, then paste it back into this menu to load the graph again."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/load.svg'/><b>Load</b><br><br>This menu contains options for loading graphs. It's a major work in progress, so all it has now is a few examples."))
            .add(new Apper.Menu.Paragraph("<img style='float:left' src='icons/help.svg'/><b>Help</b> (h)<br><br>You are here. Congratulations!"));

        this.defaultTitle = "Untitled Circuit";

        this.selection = [];
        this.drag = null;
        this.drawingWire = null;
        this.heldComponent = null;
        this.color = "";
        this.captureArea = null;

        // GET RID OF
        this.vertices = [];
        this.edges = [];
        this.firstVertex = null;
        
        this.nodes = new Map();
        this.components = [
            new Logice.Component(Logice.componentType.INPUT, 0, 0, Logice.Direction.E),
            new Logice.Component(Logice.componentType.INPUT, 0, 4, Logice.Direction.E),
            new Logice.Component(Logice.componentType.INPUT, 0, 8, Logice.Direction.E),
            new Logice.Component(Logice.componentType.XOR, 11, 2, Logice.Direction.E),
            new Logice.Component(Logice.componentType.XOR, 20, 6, Logice.Direction.E),
            new Logice.Component(Logice.componentType.AND, 20, 12, Logice.Direction.E),
            new Logice.Component(Logice.componentType.AND, 20, 18, Logice.Direction.E),
            new Logice.Component(Logice.componentType.OR, 29, 15, Logice.Direction.E),
            new Logice.Component(Logice.componentType.OUTPUT, 31, 6, Logice.Direction.E),
            new Logice.Component(Logice.componentType.OUTPUT, 31, 15, Logice.Direction.E),
        ];
        this.wires = [
            new Logice.Wire(0, 0, 4, 0),
            new Logice.Wire(4, 0, 6, 0),
            new Logice.Wire(0, 4, 2, 4),
            new Logice.Wire(2, 4, 6, 4),
            new Logice.Wire(11, 2, 13, 2),
            new Logice.Wire(13, 2, 13, 4),
            new Logice.Wire(13, 4, 15, 4),
            new Logice.Wire(0, 8, 11, 8),
            new Logice.Wire(11, 8, 15, 8),
            new Logice.Wire(20, 6, 31, 6),
            new Logice.Wire(4, 0, 4, 16),
            new Logice.Wire(4, 16, 15, 16),
            new Logice.Wire(2, 4, 2, 20),
            new Logice.Wire(2, 20, 15, 20),
            new Logice.Wire(13, 4, 13, 10),
            new Logice.Wire(13, 10, 15, 10),
            new Logice.Wire(11, 8, 11, 14),
            new Logice.Wire(11, 14, 15, 14),
            new Logice.Wire(20, 12, 22, 12),
            new Logice.Wire(22, 12, 22, 13),
            new Logice.Wire(22, 13, 24, 13),
            new Logice.Wire(20, 18, 22, 18),
            new Logice.Wire(22, 18, 22, 17),
            new Logice.Wire(22, 17, 24, 17),
            new Logice.Wire(29, 15, 31, 15),
        ];

        if (window.location.hash) {
            this.load(window.location.hash.slice(1));
        }

        // Don't render the circuit (ideally) until signals have been propagated once
        this.centerView(false);
        window.setTimeout(() => this.updateNodes());
    }

    updateNodes(iteration = 0) {
        let newNodes = new Map();
        this.components.forEach(component => {
            component.update(component.type.ins.map(input => {
                let [dx, dy] = Logice.rotateOffset(input.x, input.y, component.direction);
                let nodeKey = (component.cx + dx) + ";" + (component.cy + dy);
                if (this.nodes.has(nodeKey)) {
                    newNodes.set(nodeKey, {
                        value: 0,
                        degree: 1,
                    });
                    return this.nodes.get(nodeKey).value;
                } else {
                    return 0;
                }
            }));
            component.type.outs.forEach(output => {
                let [dx, dy] = Logice.rotateOffset(output.x, output.y, component.direction);
                let nodeKey = (component.cx + dx) + ";" + (component.cy + dy);
                // TODO: multiple outputs, bit widths
                if (newNodes.has(nodeKey)) {
                    let node = newNodes.get(nodeKey);
                    node.value = node.value === 0 ? component.state : 3;
                    node.degree++;
                } else {
                    newNodes.set(nodeKey, {
                        value: component.state,
                        degree: 1,
                    });
                }
            });
        });
        this.wires.forEach(wire => {
            let inKey = wire.x1 + ";" + wire.y1;
            let outKey = wire.x2 + ";" + wire.y2;
            if (newNodes.has(inKey)) {
                let node = newNodes.get(inKey);
                wire.state = node.value;
                node.degree++;
            } else {
                wire.state = 0;
            }
            if (newNodes.has(outKey)) {
                let node = newNodes.get(outKey);
                node.value = node.value === 0 ? wire.state : 3;
                node.degree++;
            } else {
                newNodes.set(outKey, {
                    value: wire.state,
                    degree: 1,
                });
            }
        });
        if (!Logice.nodesUnchanged(this.nodes, newNodes)) {
            this.nodes = newNodes;
            if (iteration < this.MAX_ITERATIONS) {
                window.setTimeout(() => this.updateNodes(iteration + 1));
            } else {
                this.update();
            }
        } else {
            this.update();
        }
    }

    render() {
        // TODO: better way to handle this??
        if (!this.toolbar)
            return;

        let element = this.tool === Logice.Tool.MOVE || this.tool === Logice.Tool.CAPTURE ? null : this.objectAt(this.cursorPos);
        this.color = this.tool === Logice.Tool.DRAW ? Logice.Color.GREEN_DARK
            : this.tool === Logice.Tool.ERASE ? Logice.Color.RED
                : Logice.Color.BLUE_TINT;
        let captureResizeDirection = this.tool === Logice.Tool.CAPTURE ? this.captureResizeDirection() : null;

        if (this.tool === Logice.Tool.DRAW) {
            this.drawMenu.show();
        } else {
            this.drawMenu.hide();
        }

        if (this.tool === Logice.Tool.STYLE && this.selection.length) {
            if (this.selection.some(element => element instanceof Logice.Vertex)) {
                this.widget.fillColor.element.style.display = "";
                this.widget.vertexShape.element.style.display = "";
                this.widget.vertexRadius.element.style.display = "";

                let fillColor = null;
                for (let element of this.selection) {
                    if (element instanceof Logice.Vertex)
                        if (fillColor === null) fillColor = this.colorOptions.indexOf(element.fill);
                        else if (element.fill !== fillColor) {
                            fillColor = null;
                            break;
                        }
                }
                if (fillColor === -1) fillColor = null;
                this.widget.fillColor.value = fillColor;

                let shape = null;
                for (let element of this.selection) {
                    if (element instanceof Logice.Vertex)
                        if (shape === null) shape = element.shape;
                        else if (element.shape !== shape) {
                            shape = null;
                            break;
                        }
                }
                this.widget.vertexShape.value = shape;

                let radius = null;
                for (let element of this.selection) {
                    if (element instanceof Logice.Vertex)
                        if (radius === null) radius = element.r;
                        else if (element.r !== radius) {
                            radius = null;
                            break;
                        }
                }
                this.widget.vertexRadius.value = radius ?? 10;
            } else {
                this.widget.fillColor.element.style.display = "none";
                this.widget.vertexShape.element.style.display = "none";
                this.widget.vertexRadius.element.style.display = "none";
            }

            let lineColor = null;
            for (let element of this.selection) {
                if (lineColor === null) lineColor = this.colorOptions.indexOf(element.stroke);
                else if (element.stroke !== lineColor) {
                    lineColor = null;
                    break;
                }
            }
            if (lineColor === -1) lineColor = null;
            this.widget.lineColor.value = lineColor;

            let lineWidth = null;
            for (let element of this.selection) {
                if (lineWidth === null) lineWidth = element.lineWidth;
                else if (element.lineWidth !== lineWidth) {
                    lineWidth = null;
                    break;
                }
            }
            this.widget.lineWidth.value = lineWidth ?? 4;

            this.widget.lineDash.checked = this.selection.some(element => element.lineDash.length > 1);

            this.styleMenu.show();
        } else this.styleMenu.hide();

        if (this.tool === Logice.Tool.GRID) this.gridMenu.show();
        else this.gridMenu.hide();

        if (this.tool === Logice.Tool.CAPTURE) {
            if (this.captureArea === null) this.fitCaptureTo(this.selection.length ? this.selection : this.vertices.concat(this.edges), false);
            if (this.captureArea === null) {
                this.widget.capturePrompt.show();
                this.widget.captureEmptyPreview.show();
                this.widget.captureInstructions.hide();
                this.widget.captureSelectionOnly.hide();
                this.widget.captureFitGraph.hide();
                this.widget.capturePreview.hide();
                this.widget.captureDownload.hide();

                this.widget.captureDownload.url = "";
                this.widget.captureDownload.filename = "";
            } else {
                this.widget.capturePrompt.hide();
                this.widget.captureEmptyPreview.hide();
                this.widget.captureInstructions.show();
                this.widget.captureSelectionOnly.show();
                this.widget.captureFitGraph.show();
                this.widget.capturePreview.show();
                this.widget.captureDownload.show();

                if (this.selection.length) this.widget.captureSelectionOnly.enable();
                else this.widget.captureSelectionOnly.disable().setChecked(false);

                this.widget.capturePreview.label = `Preview (${this.captureArea.w} \u00d7 ${this.captureArea.h}):`
                this.widget.capturePreview.resize(this.captureArea.w, this.captureArea.h);
                this.widget.capturePreview.ctx.resetTransform();
                this.widget.capturePreview.ctx.clearRect(0, 0, this.widget.capturePreview.canvas.width, this.widget.capturePreview.canvas.height);
                this.widget.capturePreview.ctx.translate(-this.captureArea.x, -this.captureArea.y);
                this.widget.capturePreview.ctx.lineCap = "round";
                this.widget.capturePreview.ctx.lineJoin = "round";

                // TODO: users should be able to modify resolution through capture menu
                this.edges.forEach(edge => {
                    if (this.widget.captureSelectionOnly.checked && !this.selection.includes(edge)) return;
                    edge.draw(this.widget.capturePreview.ctx, false, false, this.color);
                });
                this.vertices.forEach(vertex => {
                    if (this.widget.captureSelectionOnly.checked && !this.selection.includes(vertex)) return;
                    vertex.draw(this.widget.capturePreview.ctx, false, false, this.color);
                });

                this.widget.captureDownload.url = this.widget.capturePreview.canvas.toDataURL("image/png");
                this.widget.captureDownload.filename = `${this.title}.png`;
            }

            this.captureMenu.show();
        } else {
            this.captureMenu.hide();
            this.captureArea = null;
        }

        if (this.tool === Logice.Tool.DATA) {
            if (!this.widget.dataEditor.editing) {
                this.widget.dataEditor.text = this.graphData;
                this.widget.dataEditor.valid = true;
            }

            this.dataMenu.show();
        } else this.dataMenu.hide();

        if (this.tool === Logice.Tool.LOAD) this.loadMenu.show();
        else this.loadMenu.hide();

        if (this.tool === Logice.Tool.HELP) this.helpModal.show();
        else this.helpModal.hide();

        // Set cursor appearance
        if (this.tool === Logice.Tool.MOVE || (this.dragging && this.drag.button === 2)) {
            this.element.style.cursor = this.dragging ? "grabbing" : "";
            this.canvas.style.cursor = this.dragging ? "" : "grab";
        } else if (this.tool === Logice.Tool.CAPTURE) {
            let cursor = "";
            switch (this.drag?.captureResizeDirection ?? captureResizeDirection) {
                case Logice.Direction.N: cursor = "n-resize"; break;
                case Logice.Direction.E: cursor = "e-resize"; break;
                case Logice.Direction.S: cursor = "s-resize"; break;
                case Logice.Direction.W: cursor = "w-resize"; break;
                case Logice.Direction.NW: cursor = "nw-resize"; break;
                case Logice.Direction.NE: cursor = "ne-resize"; break;
                case Logice.Direction.SE: cursor = "se-resize"; break;
                case Logice.Direction.SW: cursor = "sw-resize"; break;
                default: cursor = this.dragging ? "grabbing" : "grab"; break;
            }
            this.element.style.cursor = this.dragging ? cursor : "";
            this.canvas.style.cursor = this.dragging ? "" : cursor;
        } else if (this.tool === Logice.Tool.INTERACT) {
            let [component, element] = this.componentElementAt(this.cursorPos);
            this.element.style.cursor = "";
            this.canvas.style.cursor = element == null ? "" : "pointer";
        } else {
            this.element.style.cursor = "";
            this.canvas.style.cursor = element == null ? "" : "pointer";
        }

        this.ctx.lineCap = "round";
        this.ctx.lineJoin = "round";

        // BACKGROUND
        
        const grid = this.grid, drawGrid = this.widget.enableGrid.checked, drawAxes = this.widget.enableAxes.checked;
        if (drawGrid || drawAxes) {
            let topLeft = this.locate(new Apper.Vector2(0, 0)), bottomRight = this.locate(new Apper.Vector2(this.width, this.height));
            const startX = this.transformX(Math.floor(topLeft.x / grid.x) * grid.x), startY = this.transformY(Math.floor(topLeft.y / grid.y) * grid.y);
            const endX = this.transformX(Math.ceil(bottomRight.x / grid.x) * grid.x), endY = this.transformY(Math.ceil(bottomRight.y / grid.y) * grid.y);
            const x0 = this.transformX(0), y0 = this.transformY(0);

            this.ctx.lineWidth = 2;
            this.ctx.setLineDash([]);

            if (drawGrid) {
                this.ctx.strokeStyle = "#fff1";
                this.ctx.beginPath();
                if (grid.x * this.zoom >= 8) for (let x = startX; x <= endX; x += grid.x * this.zoom) {
                    if (drawAxes && x === x0) continue;
                    this.ctx.moveTo(x, startY);
                    this.ctx.lineTo(x, endY);
                }
                if (grid.y * this.zoom >= 8) for (let y = startY; y <= endY; y += grid.y * this.zoom) {
                    if (drawAxes && y === y0) continue;
                    this.ctx.moveTo(startX, y);
                    this.ctx.lineTo(endX, y);
                }
                this.ctx.stroke();
            }

            if (drawAxes) {
                this.ctx.strokeStyle = "#fff3";
                this.ctx.beginPath();
                this.ctx.moveTo(startX, y0);
                this.ctx.lineTo(endX, y0);
                this.ctx.moveTo(x0, startY);
                this.ctx.lineTo(x0, endY);
                this.ctx.stroke();
            }
        }

        // AUXILLARY BELOW

        // Original element position indicators
        /*if (this.dragging && this.drag.targets.length) {
            for (let element of this.drag.targets) {
                element.draw(this.ctx, false, false, this.color, this.view);
                if (element instanceof Logice.Edge && element.cp !== null) {
                    const cp = this.view.transform(element.cp);
                    this.ctx.strokeStyle = `${this.color}77`;
                    this.ctx.lineWidth = 2;
                    this.ctx.setLineDash([]);
                    this.ctx.beginPath();
                    this.ctx.moveTo(cp.x, cp.y + 6);
                    this.ctx.lineTo(cp.x + 6, cp.y);
                    this.ctx.lineTo(cp.x, cp.y - 6);
                    this.ctx.lineTo(cp.x - 6, cp.y);
                    this.ctx.closePath();
                    this.ctx.stroke();
                }
            }
        }*/
        
        this.selection.forEach(item => {
            this.ctx.strokeStyle = `${this.color}99`;
            this.ctx.fillStyle = `${this.color}99`;
            this.ctx.setLineDash([]);
            this.ctx.lineWidth = 2 * this.view.zoom;
            item.highlightBack(this.ctx, this.view);
        });

        // COMPONENT
        
        for (let [key, node] of this.nodes) {
            if (node.degree >= 3) {
                let [x, y] = key.split(";");
                x = this.view.transformX(x * 12);
                y = this.view.transformY(y * 12);
                this.ctx.fillStyle = Logice.Wire.colorOfState(node.value);
                this.ctx.beginPath();
                this.ctx.ellipse(x, y, 6 * this.view.zoom, 6 * this.view.zoom, 0, 0, 2 * Math.PI);
                this.ctx.fill();
            }
        }
        this.wires.forEach(wire => {
            wire.draw(this.ctx, this.view);
        });
        this.components.forEach(component => {
            component.draw(this.ctx, this.view);
        });

        // AUXILLARY ABOVE

        if (this.drawingWire) {
            // Draw preview of the wire currently being drawn
            // TODO: check wire overlap
            this.ctx.strokeStyle = `${this.color}aa`;
            this.ctx.lineWidth = 2;
            this.ctx.beginPath();
            let [startX, startY] = [this.transformX(this.drawingWire.x1), this.transformY(this.drawingWire.y1)];
            let [endX, endY] = [this.transformX(this.drawingWire.x2), this.transformY(this.drawingWire.y2)];
            this.ctx.moveTo(startX, startY);
            if (this.drawingWire.verticalFirst) {
                this.ctx.lineTo(startX, endY);
            } else {
                this.ctx.lineTo(endX, startY);
            }
            this.ctx.lineTo(endX, endY);
            this.ctx.stroke();
        }

        this.selection.forEach(item => {
            this.ctx.strokeStyle = this.color;
            this.ctx.fillStyle = this.color;
            this.ctx.setLineDash([]);
            item.highlightFront(this.ctx, this.view);
        });

        if (this.dragging && (this.drag.button === 2 || this.tool === Logice.Tool.MOVE || this.tool === Logice.Tool.CAPTURE)) {
            // Draw drag start indicator
            this.ctx.strokeStyle = "#fff2";
            this.ctx.lineWidth = 2;
            this.ctx.setLineDash([]);
            this.ctx.beginPath();
            this.ctx.moveTo(this.drag.x, this.drag.y - 8);
            this.ctx.lineTo(this.drag.x, this.drag.y + 8);
            this.ctx.moveTo(this.drag.x - 8, this.drag.y);
            this.ctx.lineTo(this.drag.x + 8, this.drag.y);
            if (!this.cursorPos.equals(0, 0) && (Math.abs(this.cursorPos.x - this.drag.x) > this.REVERT_PROXIMITY || Math.abs(this.cursorPos.y - this.drag.y) > this.REVERT_PROXIMITY)) {
                this.ctx.moveTo(this.cursorPos.x, this.cursorPos.y - 8);
                this.ctx.lineTo(this.cursorPos.x, this.cursorPos.y + 8);
                this.ctx.moveTo(this.cursorPos.x - 8, this.cursorPos.y);
                this.ctx.lineTo(this.cursorPos.x + 8, this.cursorPos.y);
            }
            this.ctx.stroke();
        } else if (this.tool === Logice.Tool.DRAW && !this.cursorPos.equals(0, 0)) {
            if (this.heldComponent) {
                // Draw preview of component in hand
                this.heldComponent.cx = Math.round(this.locateX(this.cursorPos.x) / 12);
                this.heldComponent.cy = Math.round(this.locateY(this.cursorPos.y) / 12);
                this.ctx.lineWidth = 4 * this.view.zoom;
                this.ctx.strokeStyle = `${this.color}77`;
                this.ctx.setLineDash([]);
                this.ctx.stroke(this.heldComponent.highlightPath(this.view));
            }
            let pos = this.transform(this.snapToGrid(this.locate(this.cursorPos)));
            this.ctx.fillStyle = `${this.color}aa`;
            this.ctx.beginPath();
            this.ctx.ellipse(pos.x, pos.y, 5, 5, 0, 0, 2 * Math.PI);
            this.ctx.fill();
        } else if (this.dragging && !this.drag.targets.length && !this.cursorPos.equals(0, 0)) {
            // Draw selection box
            this.ctx.strokeStyle = `${this.color}77`;
            this.ctx.lineWidth = 2;
            this.ctx.setLineDash([5, 5]);
            // Could just ctx.rect() but locking the line dash at the start and end corners looks nicer
            this.ctx.beginPath();
            this.ctx.moveTo(this.drag.x, this.drag.y);
            this.ctx.lineTo(this.cursorPos.x, this.drag.y);
            this.ctx.moveTo(this.drag.x, this.drag.y);
            this.ctx.lineTo(this.drag.x, this.cursorPos.y);
            this.ctx.moveTo(this.cursorPos.x, this.cursorPos.y);
            this.ctx.lineTo(this.drag.x, this.cursorPos.y);
            this.ctx.moveTo(this.cursorPos.x, this.cursorPos.y);
            this.ctx.lineTo(this.cursorPos.x, this.drag.y);
            this.ctx.stroke();
        }

        // Capture box
        if (this.tool === Logice.Tool.CAPTURE && this.captureArea != null) {
            const rect = this.captureArea.transformed(this.view);
            const cx = Math.round(rect.cx), cy = Math.round(rect.cy);
            this.ctx.fillStyle = `${Logice.Color.ORANGE}33`;
            this.ctx.strokeStyle = `${Logice.Color.ORANGE}aa`;
            this.ctx.lineWidth = 2;
            this.ctx.setLineDash([]);
            this.ctx.beginPath();
            this.ctx.rect(rect.x, rect.y, rect.w, rect.h);
            this.ctx.fill();
            // Resize handles
            this.ctx.rect(rect.x - 6, rect.y - 6, 12, 12);
            this.ctx.rect(rect.xw - 6, rect.y - 6, 12, 12);
            this.ctx.rect(rect.xw - 6, rect.yh - 6, 12, 12);
            this.ctx.rect(rect.x - 6, rect.yh - 6, 12, 12);
            if (rect.w > 2 * this.RESIZE_PROXIMITY) {
                this.ctx.rect(cx - 6, rect.y - 6, 12, 12);
                this.ctx.rect(cx - 6, rect.yh - 6, 12, 12);
            }
            if (rect.h > 2 * this.RESIZE_PROXIMITY) {
                this.ctx.rect(rect.xw - 6, cy - 6, 12, 12);
                this.ctx.rect(rect.x - 6, cy - 6, 12, 12);
            }
            this.ctx.stroke();
            // Indicator for hovering over a resize portion
            if (this.drag?.captureResizeDirection ?? captureResizeDirection != null) {
                this.ctx.fillStyle = this.drag?.captureResizeDirection != null ? `${Logice.Color.ORANGE}aa` : `${Logice.Color.ORANGE}77`;
                this.ctx.beginPath();
                switch (this.drag?.captureResizeDirection ?? captureResizeDirection) {
                    case Logice.Direction.N: this.ctx.rect(cx - 6, rect.y - 6, 12, 12); break;
                    case Logice.Direction.E: this.ctx.rect(rect.xw - 6, cy - 6, 12, 12); break;
                    case Logice.Direction.S: this.ctx.rect(cx - 6, rect.yh - 6, 12, 12); break;
                    case Logice.Direction.W: this.ctx.rect(rect.x - 6, cy - 6, 12, 12); break;
                    case Logice.Direction.NW: this.ctx.rect(rect.x - 6, rect.y - 6, 12, 12); break;
                    case Logice.Direction.NE: this.ctx.rect(rect.xw - 6, rect.y - 6, 12, 12); break;
                    case Logice.Direction.SE: this.ctx.rect(rect.xw - 6, rect.yh - 6, 12, 12); break;
                    case Logice.Direction.SW: this.ctx.rect(rect.x - 6, rect.yh - 6, 12, 12); break;
                    default: break;
                }
                this.ctx.fill();
            }
        }
    }

    mouseDown(event) {
        if (event.button === 0 || event.button === 2) {
            if (this.dragging && event.button !== this.drag.button) return true;

            if (this.tool === Logice.Tool.INTERACT && event.button === 0) {
                let [component, element] = this.componentElementAt(event.screenPos);
                if (element?.clickAction?.(component)) {
                    window.setTimeout(() => this.updateNodes());
                }
                return true;
            }

            const canSelect = event.button !== 2 && this.tool !== Logice.Tool.MOVE && this.tool !== Logice.Tool.CAPTURE;
            let target = canSelect ? this.objectAt(event.screenPos) : null;
            let shouldUpdateNodes = false;

            if (this.tool === Logice.Tool.DRAW && event.button === 0) {
                shouldUpdateNodes = true;
            } else if (this.tool === Logice.Tool.ERASE && event.button === 0) {
                if (target != null) {
                    if (!this.selection.includes(target))
                        this.selection = [target];
                    this.deleteSelection();
                    target = null;
                    shouldUpdateNodes = true;
                }
            }

            if (target == null) {
                if (canSelect && !event.shiftKey) {
                    this.selection = [];
                }
            } else if (event.shiftKey) {
                if (this.selection.includes(target)) {
                    this.selection.splice(this.selection.indexOf(target), 1);
                } else {
                    this.selection.push(target);
                }
            } else if (!this.selection.includes(target)) {
                this.selection = [target];
            }

            let targets = event.shiftKey ? [] : this.selection.map(item => item.clone());
            let indices = event.shiftKey ? [] : this.selection.map(item =>
                item instanceof Logice.Component ?
                this.components.indexOf(item) :
                this.wires.indexOf(item));

            this.drag = {
                button: event.button,
                x: event.screenPos.x,
                y: event.screenPos.y,
                view: this.view.copy(),
                targets, indices,
                captureArea: this.captureArea,
                captureResizeDirection: this.captureResizeDirection(),
            };

            if (shouldUpdateNodes) {
                window.setTimeout(() => this.updateNodes());
            }

            return true;
        }

        return false;
    }

    mouseMove(event) {
        if (event.screenPos.equals(0, 0))
            return false;
        if (!this.dragging)
            return true;
        if (!event.leftBtn && !event.rightBtn)
            return true;

        const canSelect = this.drag.button !== 2 && this.tool !== Logice.Tool.MOVE && this.tool !== Logice.Tool.CAPTURE;
        const dx = event.screenPos.x - this.drag.x, dy = event.screenPos.y - this.drag.y;
        const revertX = Math.abs(dx) <= this.REVERT_PROXIMITY, revertY = Math.abs(dy) <= this.REVERT_PROXIMITY, revert = revertX && revertY;

        if (!canSelect && this.drag.captureResizeDirection == null) {
            this.view.center.set(revert ? this.drag.view.center : this.drag.view.center.sub(dx / this.zoom, dy / this.zoom));

        } else if (this.tool === Logice.Tool.CAPTURE) {
            let l = this.drag.captureArea.x, r = this.drag.captureArea.xw, t = this.drag.captureArea.y, b = this.drag.captureArea.yh;
            if (!revertY) {
                if (this.drag.captureResizeDirection & Logice.Direction.N)
                    t += dy / this.zoom;
                if (this.drag.captureResizeDirection & Logice.Direction.S)
                    b += dy / this.zoom;
            }
            if (!revertX) {
                if (this.drag.captureResizeDirection & Logice.Direction.E)
                    r += dx / this.zoom;
                if (this.drag.captureResizeDirection & Logice.Direction.W)
                    l += dx / this.zoom;
            }
            if (t > b) {
                this.drag.captureResizeDirection ^= Logice.Direction.N | Logice.Direction.S;
                this.drag.captureArea.y = this.drag.captureArea.yh;
                this.drag.captureArea.h = -this.drag.captureArea.h;
            }
            if (l > r) {
                this.drag.captureResizeDirection ^= Logice.Direction.E | Logice.Direction.W;
                this.drag.captureArea.x = this.drag.captureArea.xw;
                this.drag.captureArea.w = -this.drag.captureArea.w;
            }
            this.captureArea = new Apper.Rect(Math.min(l, r), Math.min(t, b), Math.abs(r - l), Math.abs(b - t));

        } else {
            for (let i = 0; i < this.drag.targets.length; i++) {
                let target = this.drag.targets[i];
                let index = this.drag.indices[i];
                let [moveX, moveY] = [Math.round(dx / (this.zoom * 12)), Math.round(dy / (this.zoom * 12))];

                if (target instanceof Logice.Component) {
                    let component = this.components[index];
                    if (this.tool !== Logice.Tool.DRAW) {
                        component.cx = target.cx + moveX;
                        component.cy = target.cy + moveY;
                    }
                } else if (target instanceof Logice.Wire) {
                    let wire = this.wires[index];
                    if (this.tool !== Logice.Tool.DRAW) {
                        wire.x1 = target.x1 + moveX;
                        wire.y1 = target.y1 + moveY;
                        wire.x2 = target.x2 + moveX;
                        wire.y2 = target.y2 + moveY;
                    }
                    /*if (this.drag.targets.length === 1) {
                        if (!edge.cp) edge.cp = new Apper.Vector2();
                        edge.cp.set(revert ? target.cp ?? this.locate(this.drag) : this.snapToGrid((target.cp ?? this.locate(this.drag)).add(dx * this.izoom, dy * this.izoom)));
    
                        if (!edge.isLoop) {
                            this.ctx.lineWidth = this.STRAIGHTEN_RADIUS * 2;
                            this.ctx.setLineDash([]);
                            this.ctx.beginPath();
                            this.ctx.moveTo(this.transformX(edge.v1.x), this.transformY(edge.v1.y));
                            this.ctx.lineTo(this.transformX(edge.v2.x), this.transformY(edge.v2.y));
                            if (this.ctx.isPointInStroke(this.transformX(edge.cp.x) * this.scale, this.transformY(edge.cp.y) * this.scale))
                                edge.cp = null;
                        }
                    } else if (edge.cp !== null && target.cp !== null) {
                        edge.cp.set(revert ? target.cp : this.snapToGrid(target.cp.add(dx * this.izoom, dy * this.izoom)));
                    }*/
                }
            }
        }

        return true;
    }

    mouseUp(event) {
        if (this.dragging && event.button === this.drag.button) {
            let target = this.drag.button === 2 || this.tool === Logice.Tool.MOVE || this.tool === Logice.Tool.CAPTURE ? null : this.objectAt(this.cursorPos);

            if (this.tool === Logice.Tool.DRAW) {
                if (this.firstVertex !== null && this.firstVertex !== target) {
                    if (event.ctrlKey) {
                        let loop = new Logice.Edge({ v1: this.firstVertex, v2: this.firstVertex, cp: this.snapToGrid(this.locate(this.cursorPos)) });
                        this.edges.push(loop);
                        this.selection = [loop];
                    } else {
                        if (target instanceof Logice.Vertex) {
                            this.edges.push(new Logice.Edge({ v1: this.firstVertex, v2: target }));
                            this.selection = [target];
                        } else {
                            let placed = new Logice.Vertex(this.snapToGrid(this.locate(this.cursorPos)));
                            this.vertices.push(placed);
                            this.edges.push(new Logice.Edge({ v1: this.firstVertex, v2: placed }));
                            this.selection = [placed];
                        }
                    }
                }
                this.firstVertex = null;

            } else if (this.tool === Logice.Tool.CAPTURE) {
                this.captureArea = this.captureArea.normalized();
                if (this.captureArea.w < 1) {
                    this.captureArea.w = 1;
                    if (this.drag.captureResizeDirection & Logice.Direction.W) this.captureArea.x -= 1;
                }
                if (this.captureArea.h < 1) {
                    this.captureArea.h = 1;
                    if (this.drag.captureResizeDirection & Logice.Direction.N) this.captureArea.y -= 1;
                }

            } else if (this.drag.button !== 2 && this.tool !== Logice.Tool.MOVE && !this.drag.targets.length) {
                if (!event.shiftKey)
                    this.selection = [];
                let selectBox = new Apper.Rect(this.drag.x, this.drag.y, this.cursorPos.x - this.drag.x, this.cursorPos.y - this.drag.y).located(this.view).normalized();
                /*if (selectBox.area) this.vertices.filter(vertex => {
                    return selectBox.intersects(new Apper.Rect(vertex.x - vertex.r, vertex.y - vertex.r, 2 * vertex.r, 2 * vertex.r));  // FIXME: doesn't account for shape
                }).concat(this.edges.filter(edge => {
                    if (selectBox.contains(edge.v1) || selectBox.contains(edge.v2)) return true;
                    if (edge.cp === null) {
                        return Logice.linesIntersect(edge.v1, edge.v2, selectBox.xy, selectBox.xwy)
                            || Logice.linesIntersect(edge.v1, edge.v2, selectBox.xwy, selectBox.xwyh)
                            || Logice.linesIntersect(edge.v1, edge.v2, selectBox.xwyh, selectBox.xyh)
                            || Logice.linesIntersect(edge.v1, edge.v2, selectBox.xyh, selectBox.xy);
                    } else if (edge.isLoop) {
                        let center = edge.cp.add(edge.v1).mul(0.5),
                            radius = 0.5 * Math.hypot(edge.cp.x - edge.v1.x, edge.cp.y - edge.v1.y);
                        if (center.x + radius < selectBox.x || selectBox.xw < center.x - radius || center.y + radius < selectBox.y || selectBox.yh < center.y - radius)
                            return false;
                        let distX = Math.max(Math.abs(center.x - selectBox.cx) - 0.5 * selectBox.w, 0),
                            distY = Math.max(Math.abs(center.y - selectBox.cy) - 0.5 * selectBox.h, 0);
                        return distX * distX + distY * distY <= radius * radius
                            && (Math.hypot(selectBox.x - center.x, selectBox.y - center.y) >= radius
                                || Math.hypot(selectBox.xw - center.x, selectBox.y - center.y) >= radius
                                || Math.hypot(selectBox.xw - center.x, selectBox.yh - center.y) >= radius
                                || Math.hypot(selectBox.x - center.x, selectBox.yh - center.y) >= radius);
                    } else {
                        let l = Math.min(edge.v1.x, edge.cp.x, edge.v2.x),
                            r = Math.max(edge.v1.x, edge.cp.x, edge.v2.x),
                            t = Math.min(edge.v1.y, edge.cp.y, edge.v2.y),
                            b = Math.max(edge.v1.y, edge.cp.y, edge.v2.y);
                        if (!selectBox.intersects(new Apper.Rect(l, t, r - l, b - t))) return false;
                        let v1 = new Apper.Vector2(edge.v1);
                        let v2 = new Apper.Vector2(edge.v2);
                        for (let time = 0; time <= 1; time += this.CURVE_SAMPLE_INTERVAL) {
                            if (selectBox.contains(Logice.curveQ(time, v1, edge.cp, v2))) return true;
                        }
                        return false;
                    }
                })).forEach(element => {
                    if (!this.selection.includes(element)) this.selection.push(element);
                });*/

                if (this.tool === Logice.Tool.ERASE) {
                    this.deleteSelection();
                }
            }

            window.setTimeout(() => this.updateNodes());

            this.drag = null;
            return true;
        }
    }

    openContextMenu(event) {
        // TODO: custom context menu
        return true;
    }

    scrollWheel(event) {
        if (!event.dy) return false;
        this.view.changeZoom(-event.dy * this.WHEEL_ZOOM_COEFF, this.cursorPos.equals(0, 0) ? null : this.cursorPos);
        return true;
    }

    keyDown(event) {
        switch (event.key) {
            case "keyv": this.tool = Logice.Tool.SELECT; return true;
            case "keym": this.tool = Logice.Tool.MOVE; return true;
            case "keyd": this.tool = Logice.Tool.DRAW; return true;
            case "keyx": this.tool = Logice.Tool.ERASE; return true;
            case "keys": this.tool = Logice.Tool.STYLE; return true;
            case "keyg": this.tool = Logice.Tool.GRID; return true;
            case "keyh": this.tool = Logice.Tool.HELP; return true;
            case "keya":
                if (event.ctrlKey) return event.shiftKey ? this.deselectAll() : this.selectAll();
                break;
            case "escape":
                this.firstVertex = null;
                return true;
            case "backspace":
            case "delete":
                return this.deleteSelection();
            case "period":
            case "numpaddecimal":
                this.centerView();
                return true;
            case "equal":
            case "numpadadd":
                this.view.changeZoom(this.KEY_ZOOM_AMOUNT);
                return true;
            case "minus":
            case "numpadsubtract":
                this.view.changeZoom(-this.KEY_ZOOM_AMOUNT);
                return true;
            case "digit0":
            case "numpad0":
                this.zoom = 1;
                return true;
        }

        return false;
    }

    snapToGrid(pos) {
        if (this.widget.enableGrid.checked)
            return new Apper.Vector2(Math.round(pos.x / this.grid.x) * this.grid.x, Math.round(pos.y / this.grid.y) * this.grid.y);
        else return pos;
    }

    componentAt(pos) {
        for (let i = this.components.length - 1; i >= 0; i--) {
            if (this.components[i].contains(this.ctx, this.view, pos)) {
                return this.components[i];
            }
        }
        return null;
    }

    wireAt(pos) {
        for (let i = this.wires.length - 1; i >= 0; i--) {
            if (this.wires[i].contains(this.ctx, this.view, pos)) {
                return this.wires[i];
            }
        }
        return null;
    }

    objectAt(pos) {
        return this.componentAt(pos) ?? this.wireAt(pos);
    }

    componentElementAt(pos) {
        let component = this.componentAt(pos);
        return [component, component?.elementAt(this.ctx, this.view, pos)];
    }

    selectAll() {
        this.selection = this.components.concat(this.wires);

        return true;
    }

    deselectAll() {
        this.selection = [];

        return true;
    }

    deleteSelection() {
        if (!this.selection.length)
            return false;

        this.selection.forEach(selected => {
            if (selected instanceof Logice.Component && this.components.includes(selected)) {
                this.components.splice(this.components.indexOf(selected), 1);
            } else if (selected instanceof Logice.Wire && this.wires.includes(selected)) {
                this.wires.splice(this.wires.indexOf(selected), 1);
            }
        });
        this.selection = [];

        return true;
    }

    centerView(refresh = true) {
        if (this.components.length) {
            let l = Infinity, r = -Infinity, t = Infinity, b = -Infinity;
            this.components.forEach(component => {
                if (component.cx < l) l = component.cx;
                if (component.cx > r) r = component.cx;
                if (component.cy < t) t = component.cy;
                if (component.cy > b) b = component.cy;
            });
            this.wires.forEach(wire => {
                if (wire.x1 < l) l = wire.x1;
                if (wire.x1 > r) r = wire.x1;
                if (wire.y1 < t) t = wire.y1;
                if (wire.y1 > b) b = wire.y1;
                if (wire.x2 < l) l = wire.x2;
                if (wire.x2 > r) r = wire.x2;
                if (wire.y2 < t) t = wire.y2;
                if (wire.y2 > b) b = wire.y2;
            });
            this.view.center.set(0.5 * 12 * (l + r), 0.5 * 12 * (t + b));
        } else {
            this.view.center.set(0, 0);
        }

        if (refresh)
            this.update();
    }

    fitCaptureTo(elements, refresh = true) {
        if (elements.length) {
            let l = Infinity, r = -Infinity, t = Infinity, b = -Infinity;
            elements.forEach(element => {
                if (element instanceof Logice.Vertex) {
                    if (element.x - element.margin < l) l = element.x - element.margin;
                    if (element.x + element.margin > r) r = element.x + element.margin;
                    if (element.y - element.margin < t) t = element.y - element.margin;
                    if (element.y + element.margin > b) b = element.y + element.margin;
                } else if (element instanceof Logice.Edge) {
                    if (element.cp !== null) {
                        if (element.isLoop) {
                            let center = element.cp.add(element.v1).mul(0.5),
                                radius = 0.5 * Math.hypot(element.cp.x - element.v1.x, element.cp.y - element.v1.y);
                            if (center.x - radius - element.margin < l) l = center.x - radius - element.margin;
                            if (center.x + radius + element.margin > r) r = center.x + radius + element.margin;
                            if (center.y - radius - element.margin < t) t = center.y - radius - element.margin;
                            if (center.y + radius + element.margin > b) b = center.y + radius + element.margin;
                        } else {
                            if (element.cp.x < element.v1.x && element.cp.x < element.v2.x || element.cp.x > element.v1.x && element.cp.x > element.v2.x) {
                                let vx = Logice.curveQVertX(element.v1, element.cp, element.v2);
                                if (vx - element.margin < l) l = vx - element.margin;
                                if (vx + element.margin > r) r = vx + element.margin;
                            }
                            if (element.cp.y < element.v1.y && element.cp.y < element.v2.y || element.cp.y > element.v1.y && element.cp.y > element.v2.y) {
                                let vy = Logice.curveQVertY(element.v1, element.cp, element.v2);
                                if (vy - element.margin < t) t = vy - element.margin;
                                if (vy + element.margin > b) b = vy + element.margin;
                            }
                        }
                    } else {
                        if (element.v1.x - element.margin < l) l = element.v1.x - element.margin;
                        if (element.v1.x + element.margin > r) r = element.v1.x + element.margin;
                        if (element.v1.y - element.margin < t) t = element.v1.y - element.margin;
                        if (element.v1.y + element.margin > b) b = element.v1.y + element.margin;
                        if (element.v2.x - element.margin < l) l = element.v2.x - element.margin;
                        if (element.v2.x + element.margin > r) r = element.v2.x + element.margin;
                        if (element.v2.y - element.margin < t) t = element.v2.y - element.margin;
                        if (element.v2.y + element.margin > b) b = element.v2.y + element.margin;
                    }
                }
            });
            this.captureArea = new Apper.Rect(Math.round(l), Math.round(t), Math.round(r - l), Math.round(b - t));
        } else {
            this.captureArea = null;
        }

        if (refresh) {
            this.update();
        }
    }

    captureResizeDirection() {
        if (this.captureArea === null) return null;

        const area = this.captureArea.transformed(this.view);
        let broadArea = new Apper.Rect(
            area.x - this.RESIZE_PROXIMITY,
            area.y - this.RESIZE_PROXIMITY,
            area.w + 2 * this.RESIZE_PROXIMITY,
            area.h + 2 * this.RESIZE_PROXIMITY);

        if (broadArea.contains(this.cursorPos)) {
            let dw = Math.abs(area.x - this.cursorPos.x),
                de = Math.abs(area.xw - this.cursorPos.x),
                dn = Math.abs(area.y - this.cursorPos.y),
                ds = Math.abs(area.yh - this.cursorPos.y);
            let west = dw <= this.RESIZE_PROXIMITY && dw < de,
                east = de <= this.RESIZE_PROXIMITY && de < dw,
                north = dn <= this.RESIZE_PROXIMITY && dn < ds,
                south = ds <= this.RESIZE_PROXIMITY && ds < dn;
            if (north) return west ? Logice.Direction.NW : east ? Logice.Direction.NE : Logice.Direction.N;
            if (south) return west ? Logice.Direction.SW : east ? Logice.Direction.SE : Logice.Direction.S;
            if (west) return Logice.Direction.W;
            if (east) return Logice.Direction.E;
        }

        return area.contains(this.cursorPos) ? Logice.Direction.C : null;
    }

    load(url) {
        this.title = "Loading...";

        let trueURL = url.startsWith("examples") ? this.HOST_URL + url : url;

        fetch(trueURL)
            .then(response => response.json(), error => {
                console.error(error);
                if (error instanceof TypeError) error = "The circuit URL could not be reached.";
                this.showError(error);
                this.title = this.defaultTitle;
            }).then(data => {
                window.location.hash = url;
                this.loadFromData(data, true);
            }, error => {
                console.error(error);
                if (error instanceof SyntaxError) error = "The circuit at this URL could not be read.";
                this.showError(error);
                this.title = this.defaultTitle;
            });
    }

    loadFromData(data, reset = false) {
        this.vertices = data.vertices.map(vertex => new Logice.Vertex(vertex));
        this.edges = data.edges.map(edge => new Logice.Edge({ ...edge, v1: this.vertices[edge.v1], v2: this.vertices[edge.v2] }));
        this.title = data.title ?? this.defaultTitle;

        if (reset) {
            this.centerView();
        } else {
            this.update();
        }
    }

    get graphData() {
        let vertices = this.vertices.map(vertex => ({
            x: vertex.x, y: vertex.y,
            r: vertex.r === 10 ? undefined : vertex.r,
            fill: vertex.fill === Logice.Color.DARK ? undefined : vertex.fill,
            stroke: vertex.stroke === Logice.Color.LIGHT ? undefined : vertex.stroke,
            lineWidth: vertex.lineWidth === 4 ? undefined : vertex.lineWidth,
            lineDash: !vertex.lineDash.length ? undefined : vertex.lineDash.slice(),
            shape: vertex.shape === 0 ? undefined : vertex.shape
        }));

        let edges = this.edges.map(edge => ({
            v1: this.vertices.indexOf(edge.v1), v2: this.vertices.indexOf(edge.v2),
            cp: edge.cp === null ? undefined : [edge.cp.x, edge.cp.y],
            stroke: edge.stroke === Logice.Color.DARK ? undefined : edge.stroke,
            lineWidth: edge.lineWidth === 4 ? undefined : edge.lineWidth,
            lineDash: !edge.lineDash.length ? undefined : edge.lineDash.slice(),
        }));

        return JSON.stringify({ title: this.title, vertices, edges });
    }

}


Logice.rotateOffset = (x, y, direction) => {
    switch (direction) {
        case Logice.Direction.N:
            return [y, -x];
        case Logice.Direction.E:
        default:
            return [x, y];
        case Logice.Direction.S:
            return [-y, x];
        case Logice.Direction.W:
            return [-x, -y];
    }
};

Logice.nodesUnchanged = (nodes, newNodes) => {
    if (nodes.size !== newNodes.size) {
        return false;
    }
    for (let [key, node] of newNodes) {
        if (!nodes.has(key) || nodes.get(key).value !== node.value) {
            return false;
        }
    }
    return true;
};


// Position at t (quadratic beziér): t²r + 2t(1-t)q + (1-t)²p
Logice.curveQ = (t, p, q, r) => r.mul(t * t).add(q.mul(2 * t * (1 - t))).add(p.mul((1 - t) * (1 - t)));
// Velocity at t (quadratic beziér): 2tr + (2(1-t)-2t)q - 2(1-t)p
Logice.curveQDelta = (t, p, q, r) => r.mul(2 * t).add(q.mul(2 * (1 - t) - 2 * t)).sub(p.mul(2 * (1 - t)));
Logice.curveQVertX = (p, q, r) => p.x - 2 * q.x + r.x === 0 ? null : (-q.x * q.x + 2 * q.x * r.x - r.x * r.x) / (p.x - 2 * q.x + r.x) + r.x;
Logice.curveQVertY = (p, q, r) => p.y - 2 * q.y + r.y === 0 ? null : (-q.y * q.y + 2 * q.y * r.y - r.y * r.y) / (p.y - 2 * q.y + r.y) + r.y;

Logice.linesIntersect = (p1, q1, p2, q2) =>
    Logice.linesIntersect.orient(p1, q1, p2) != Logice.linesIntersect.orient(p1, q1, q2)
    && Logice.linesIntersect.orient(p2, q2, p1) != Logice.linesIntersect.orient(p2, q2, q1);
Logice.linesIntersect.orient = (p1, p2, p3) => Math.sign((p2.y - p1.y) * (p3.x - p2.x) - (p2.x - p1.x) * (p3.y - p2.y));

Logice.fillIcon = (color) => color === "transparent" ? "icons/transparent.svg" : `
  <svg version="1.1" width="32" height="32" xmlns="http://www.w3.org/2000/svg">
      <ellipse fill="${color}" stroke="none" cx="16" cy="16" rx="9" ry="9"/>
  </svg>`;
Logice.strokeIcon = (color) => color === "transparent" ? "icons/transparent.svg" : `
  <svg version="1.1" width="32" height="32" xmlns="http://www.w3.org/2000/svg">
      <ellipse fill="none" stroke="${color}" stroke-width="2" cx="16" cy="16" rx="8" ry="8"/>
  </svg>`;


Logice.Tool = {
    INTERACT: new Apper.Tool("interact", "Interact", "icons/interact.svg", "keyi", "i"),
    SELECT: new Apper.Tool("select", "Select", "icons/select.svg", "keyv", "v"),
    MOVE: new Apper.Tool("move", "Move", "icons/move.svg", "keym", "m"),
    DRAW: new Apper.Tool("draw", "Draw", "icons/draw.svg", "keyd", "d"),
    ERASE: new Apper.Tool("erase", "Erase", "icons/erase.svg", "keyx", "x"),
    STYLE: new Apper.Tool("style", "Style", "icons/style.svg", "keys", "s"),
    GRID: new Apper.Tool("grid", "Grid", "icons/grid.svg", "keyg", "g"),
    CAPTURE: new Apper.Tool("capture", "Capture", "icons/capture.svg"),
    DATA: new Apper.Tool("data", "View Data", "icons/data.svg"),
    LOAD: new Apper.Tool("load", "Load", "icons/load.svg"),
    HELP: new Apper.Tool("help", "Help", "icons/help.svg", "keyh", "h"),
};

Logice.Direction = {
    C: 0x1111,
    N: 0x0001, E: 0x0010, S: 0x0100, W: 0x1000,
    NE: 0x1010, NW: 0x1001, SE: 0x0110, SW: 0x1100,
};

Logice.Shape = {
    CIRCLE: 0,
    SQUARE: 1,
    DIAMOND: 2,
};

Logice.Color = {
    LIGHT: "#cccccc",
    MEDIUM: "#999999",
    DARK: "#666666",
    RED: "#e84b33",
    RED_TINT: "#f46b30",
    ORANGE: "#ff9421",
    ORANGE_TINT: "#ffb128",
    YELLOW: "#ffd731",
    YELLOW_TINT: "#ffef42",
    GREEN: "#39d12a",
    GREEN_TINT: "#72e02d",
    GREEN_DARK: "#23a54c",
    CYAN: "#16dbbd",
    CYAN_TINT: "#58ebdf",
    BLUE: "#39a0fa",
    BLUE_TINT: "#43bdff",
    PURPLE: "#b762f0",
    PURPLE_TINT: "#e074f7",
    PINK: "#ff5ed5",
    PINK_TINT: "#ff83c6",
};


Logice.ComponentType = class {

    constructor(name, display, width, height, cx, cy, defaultData, ins, outs, update, genPath, elements) {
        this.name = name;
        this.display = display;
        this.width = width;
        this.height = height;
        this.cx = cx;
        this.cy = cy;
        this.defaultData = defaultData ?? {};
        this.ins = ins ?? [];
        this.outs = outs ?? [];
        this.update = update;
        this.genPath = genPath;
        this.elements = elements ?? [];
    }

};

Logice.ComponentType.Element = class {

    constructor(draw, clickPath = null, clickAction = null) {
        this.draw = draw;
        this.clickPath = clickPath;
        this.clickAction = clickAction;
    }

};

Logice.componentType = {
    INPUT: new Logice.ComponentType("input", "Input", 36, 24, 36, 12, { state: 1 }, [], [{ x: 0, y: 0 }], (data, input) => {
        return data.state;
    }, (path, x, y, s) => {
        path.moveTo(x(-36, -12), y(-36, -12));
        path.lineTo(x(-36, 12), y(-36, 12));
        path.lineTo(x(-12, 12), y(-12, 12));
        path.lineTo(x(0, 0), y(0, 0));
        path.lineTo(x(-12, -12), y(-12, -12));
        path.closePath();
    }, [new Logice.ComponentType.Element((self, ctx, x, y, s) => {
        ctx.strokeStyle = Logice.Wire.colorOfState(self.data.state);
        ctx.fillStyle = Logice.Wire.colorOfState(self.data.state) + "88";
        ctx.beginPath();
        ctx.arc(x(-24, 0), y(-24, 0), 6 * s, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
    }, (path, x, y, s) => {
        path.moveTo(x(-30, -12), y(-30, -12));
        path.lineTo(x(-30, 12), y(-30, 12));
        path.lineTo(x(-18, 12), y(-18, 12));
        path.lineTo(x(-18, -12), y(-18, -12));
        path.closePath();
    }, (self) => (self.data.state = self.data.state === 2 ? 1 : 2, true))]),
    OUTPUT: new Logice.ComponentType("output", "Output", 36, 24, 0, 12, null, [{ x: 0, y: 0 }], [], (data, input) => {
        return input[0];
    }, (path, x, y, s) => {
        path.moveTo(x(0, -12), y(0, -12));
        path.lineTo(x(0, 12), y(0, 12));
        path.lineTo(x(24, 12), y(24, 12));
        path.lineTo(x(36, 0), y(36, 0));
        path.lineTo(x(24, -12), y(24, -12));
        path.closePath();
    }, [new Logice.ComponentType.Element((self, ctx, x, y, s) => {
        ctx.strokeStyle = Logice.Wire.colorOfState(self.state);
        ctx.fillStyle = Logice.Wire.colorOfState(self.state);;
        ctx.beginPath();
        ctx.moveTo(x(6, -6), y(6, -6));
        ctx.lineTo(x(18, -6), y(18, -6));
        ctx.lineTo(x(18, 6), y(18, 6));
        ctx.lineTo(x(6, 6), y(6, 6));
        ctx.closePath();
        ctx.stroke();
    })]),
    NOT: new Logice.ComponentType("not", "NOT Gate", 36, 24, 36, 12, null, [{ x: -3, y: 0 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? 2 : input[0] === 2 ? 1 : input[0];
    }, (path, x, y, s) => {
        path.arc(x(-6, 0), y(-6, 0), 6 * s, 0, 2 * Math.PI);
        path.moveTo(x(-36, -12), y(-36, -12));
        path.lineTo(x(-36, 12), y(-36, 12));
        path.lineTo(x(-12, 0), y(-12, 0));
        path.closePath();
    }, null),
    AND: new Logice.ComponentType("and", "AND Gate", 60, 60, 60, 30, null, [{ x: -5, y: -2 }, { x: -5, y: 2 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? (input[1] === 1 || input[1] === 2 ? 1 : 3) : input[0] === 2 ? (input[1] === 1 || input[1] === 2 ? input[1] : 3) : 3;
    }, (path, x, y, s) => {
        path.moveTo(x(-60, -30), y(-60, -30));
        path.lineTo(x(-60, 30), y(-60, 30));
        path.lineTo(x(-30, 30), y(-30, 30));
        path.arcTo(x(0, 30), y(0, 30), x(0, 0), y(0, 0), 30 * s);
        path.arcTo(x(0, -30), y(0, -30), x(-30, -30), y(-30, -30), 30 * s);
        path.closePath();
    }, null),
    OR: new Logice.ComponentType("or", "OR Gate", 60, 60, 60, 30, null, [{ x: -5, y: -2 }, { x: -5, y: 2 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? (input[1] === 1 || input[1] === 2 ? input[1] : 3) : input[0] === 2 ? (input[1] === 1 || input[1] === 2 ? 2 : 3) : 3;
    }, (path, x, y, s) => {
        path.moveTo(x(-60, -30), y(-60, -30));
        path.quadraticCurveTo(x(-48, 0), y(-48, 0), x(-60, 30), y(-60, 30));
        path.quadraticCurveTo(x(-12, 30), y(-12, 30), x(0, 0), y(0, 0));
        path.quadraticCurveTo(x(-12, -30), y(-12, -30), x(-60, -30), y(-60, -30));
    }, null),
    XOR: new Logice.ComponentType("xor", "XOR Gate", 60, 60, 60, 30, null, [{ x: -5, y: -2 }, { x: -5, y: 2 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? (input[1] === 1 || input[1] === 2 ? input[1] : 3) : input[0] === 2 ? (input[1] === 1 ? 2 : input[1] === 2 ? 1 : 3) : 3;
    }, (path, x, y, s) => {
        path.moveTo(x(-60, -30), y(-60, -30));
        path.quadraticCurveTo(x(-48, 0), y(-48, 0), x(-60, 30), y(-60, 30));
        path.moveTo(x(-48, -30), y(-48, -30));
        path.quadraticCurveTo(x(-36, 0), y(-36, 0), x(-48, 30), y(-48, 30));
        path.quadraticCurveTo(x(-12, 30), y(-12, 30), x(0, 0), y(0, 0));
        path.quadraticCurveTo(x(-12, -30), y(-12, -30), x(-48, -30), y(-48, -30));
    }, null),
    NAND: new Logice.ComponentType("nand", "NAND Gate", 72, 60, 72, 30, null, [{ x: -6, y: -2 }, { x: -6, y: 2 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? (input[1] === 1 || input[1] === 2 ? 2 : 3) : input[0] === 2 ? (input[1] === 1 ? 2 : input[1] === 2 ? 1 : 3) : 3;
    }, (path, x, y, s) => {
        path.arc(x(-6, 0), y(-6, 0), 6 * s, 0, 2 * Math.PI);
        path.moveTo(x(-72, -30), y(-72, -30));
        path.lineTo(x(-72, 30), y(-72, 30));
        path.lineTo(x(-42, 30), y(-42, 30));
        path.arcTo(x(-12, 30), y(-12, 30), x(-12, 0), y(-12, 0), 30 * s);
        path.arcTo(x(-12, -30), y(-12, -30), x(-42, -30), y(-42, -30), 30 * s);
        path.closePath();
    }, null),
    NOR: new Logice.ComponentType("nor", "NOR Gate", 72, 60, 72, 30, null, [{ x: -6, y: -2 }, { x: -6, y: 2 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? (input[1] === 1 ? 2 : input[1] === 2 ? 1 : 3) : input[0] === 2 ? (input[1] === 1 || input[1] === 2 ? 1 : 3) : 3;
    }, (path, x, y, s) => {
        path.arc(x(-6, 0), y(-6, 0), 6 * s, 0, 2 * Math.PI);
        path.moveTo(x(-72, -30), y(-72, -30));
        path.quadraticCurveTo(x(-60, 0), y(-60, 0), x(-72, 30), y(-72, 30));
        path.quadraticCurveTo(x(-24, 30), y(-24, 30), x(-12, 0), y(-12, 0));
        path.quadraticCurveTo(x(-24, -30), y(-24, -30), x(-72, -30), y(-72, -30));
    }, null),
    XNOR: new Logice.ComponentType("xnor", "XNOR Gate", 72, 60, 72, 30, null, [{ x: -6, y: -2 }, { x: -6, y: 2 }], [{ x: 0, y: 0 }], (data, input) => {
        return input[0] === 1 ? (input[1] === 1 ? 2 : input[1] === 2 ? 1 : 3) : input[0] === 2 ? (input[1] === 1 || input[1] === 2 ? input[1] : 3) : 3;
    }, (path, x, y, s) => {
        path.arc(x(-6, 0), y(-6, 0), 6 * s, 0, 2 * Math.PI);
        path.moveTo(x(-72, -30), y(-72, -30));
        path.quadraticCurveTo(x(-60, 0), y(-60, 0), x(-72, 30), y(-72, 30));
        path.moveTo(x(-60, -30), y(-60, -30));
        path.quadraticCurveTo(x(-48, 0), y(-48, 0), x(-60, 30), y(-60, 30));
        path.quadraticCurveTo(x(-24, 30), y(-24, 30), x(-12, 0), y(-12, 0));
        path.quadraticCurveTo(x(-24, -30), y(-24, -30), x(-60, -30), y(-60, -30));
    }, null),
};


Logice.Component = class {

    constructor(type, cx, cy, direction, state = null, data = {}) {
        this.type = type;
        this.cx = cx;
        this.cy = cy;
        this.direction = direction;
        this.data = { ...type.defaultData, ...data };
        this.input = null;
        this.state = state;
    }

    clone() {
        return new Logice.Component(this.type, this.cx, this.cy, this.direction, this.state, { ...this.data });
    }

    xFunction(view) {
        switch (this.direction) {
            case Logice.Direction.N:
                return (x, y) => view.transformX(12 * this.cx + y);
            case Logice.Direction.E:
            default:
                return (x, y) => view.transformX(12 * this.cx + x);
            case Logice.Direction.S:
                return (x, y) => view.transformX(12 * this.cx - y);
            case Logice.Direction.W:
                return (x, y) => view.transformX(12 * this.cx - x);
        }
    }

    yFunction(view) {
        switch (this.direction) {
            case Logice.Direction.N:
                return (x, y) => view.transformY(12 * this.cy - x);
            case Logice.Direction.E:
            default:
                return (x, y) => view.transformY(12 * this.cy + y);
            case Logice.Direction.S:
                return (x, y) => view.transformY(12 * this.cy + x);
            case Logice.Direction.W:
                return (x, y) => view.transformY(12 * this.cy - y);
        }
    }

    update(input) {
        this.input = input;
        this.state = this.type.update(this.data, input);
    }

    path(view) {
        view ??= new Apper.Viewport();
        let path = new Path2D();
        this.type.genPath?.(path, this.xFunction(view), this.yFunction(view), view.zoom);
        return path;
    }

    boundingPath(view, stroke = false) {
        view ??= new Apper.Viewport();
        let offset = stroke ? 2 * view.zoom : 0;
        let [x, y] = [this.xFunction(view), this.yFunction(view)];
        let [tlx, tly, brx, bry] = [
            x(-this.type.cx - offset, -this.type.cy - offset),
            y(-this.type.cx - offset, -this.type.cy - offset),
            x(-this.type.cx + this.type.width + offset, -this.type.cy + this.type.height + offset),
            y(-this.type.cx + this.type.width + offset, -this.type.cy + this.type.height + offset),
        ];
        let path = new Path2D();
        path.moveTo(tlx, tly);
        path.lineTo(tlx, bry);
        path.lineTo(brx, bry);
        path.lineTo(brx, tly);
        path.closePath();
        return path;
    }

    highlightBack(ctx, view) {
        view ??= new Apper.Viewport();
        ctx.lineWidth = 4 * view.zoom + 4;
        ctx.stroke(this.path(view));
    }

    highlightFront(ctx, view) {
        view ??= new Apper.Viewport();
        ctx.beginPath();
        ctx.rect(view.transformX(this.cx * 12 - 4), view.transformY(this.cy * 12 - 4), 8 * view.zoom, 8 * view.zoom);
        ctx.fill();
        ctx.stroke();
    }

    draw(ctx, view) {
        view ??= new Apper.Viewport();
        ctx.lineWidth = 3 * view.zoom;
        ctx.strokeStyle = Logice.Color.LIGHT;
        ctx.setLineDash([]);
        const path = this.path(view);
        ctx.stroke(path);
        this.type.elements.forEach(element => element.draw?.(this, ctx, this.xFunction(view), this.yFunction(view), view.zoom));
    }

    contains(ctx, view, pos) {
        view ??= new Apper.Viewport();
        return ctx.isPointInPath(this.boundingPath(view), pos.x * view.scale, pos.y * view.scale);
    }

    elementAt(ctx, view, pos) {
        view ??= new Apper.Viewport();
        let [x, y] = [this.xFunction(view), this.yFunction(view)];
        for (let element of this.type.elements) {
            if (typeof element.clickPath === "function") {
                let path = new Path2D();
                element.clickPath(path, x, y, view.zoom);
                if (ctx.isPointInPath(path, pos.x * view.scale, pos.y * view.scale)) {
                    return element;
                }
            }
        }
        return null;
    }

};


Logice.Wire = class {

    constructor(x1, y1, x2, y2, state = 0) {
        this.x1 = x1;
        this.y1 = y1;
        this.x2 = x2;
        this.y2 = y2;
        this.state = state;
    }

    clone() {
        return new Logice.Wire(this.x1, this.y1, this.x2, this.y2, this.state);
    }

    path(view) {
        view ??= new Apper.Viewport();
        let path = new Path2D();
        path.moveTo(view.transformX(12 * this.x1), view.transformY(12 * this.y1));
        path.lineTo(view.transformX(12 * this.x2), view.transformY(12 * this.y2));
        return path;
    }

    boundingPath(view) {
        return this.path(view);
    }

    highlightBack(ctx, view) {
        view ??= new Apper.Viewport();
        ctx.lineWidth = 4 * view.zoom + 4;
        ctx.stroke(this.path(view));
    }

    highlightFront(ctx, view) {
        view ??= new Apper.Viewport();
        const SQRT_3 = 1.73205080757;
        let [midX, midY] = [view.transformX(12 * 0.5 * (this.x1 + this.x2)), view.transformY(12 * 0.5 * (this.y1 + this.y2))];
        let [dirX, dirY] = [this.x2 - this.x1, this.y2 - this.y1];
        let normalize = view.zoom * 8 / Math.sqrt(dirX * dirX + dirY * dirY);
        dirX *= normalize;
        dirY *= normalize;
        ctx.beginPath();
        ctx.moveTo(midX + dirX * -1/3 - dirY * 1/SQRT_3, midY + dirY * -1/3 + dirX * 1/SQRT_3);
        ctx.lineTo(midX + dirX * 2/3, midY + dirY * 2/3);
        ctx.lineTo(midX + dirX * -1/3 - dirY * -1/SQRT_3, midY + dirY * -1/3 + dirX * -1/SQRT_3);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
    }

    draw(ctx, view) {
        view ??= new Apper.Viewport();
        ctx.lineWidth = 3 * view.zoom;
        ctx.strokeStyle = Logice.Wire.colorOfState(this.state);
        ctx.setLineDash(this.state <= 0 ? [6 * view.zoom, 6 * view.zoom] : []);
        ctx.lineDashOffset = 3 * view.zoom;
        const path = this.path(view);
        ctx.stroke(path);
    }

    contains(ctx, view, pos) {
        view ??= new Apper.Viewport();
        ctx.lineWidth = 4 * view.zoom + 4;
        ctx.setLineDash([]);
        const path = this.path(view);
        return ctx.isPointInStroke(path, pos.x * view.scale, pos.y * view.scale);
    }

};

Logice.Wire.colors = [
    Logice.Color.DARK,
    Logice.Color.DARK,
    Logice.Color.GREEN_TINT,
    Logice.Color.RED,
];

Logice.Wire.colorOfState = (state) => Logice.Wire.colors[state] ?? Logice.Wire.colors[3];


Logice.Vertex = class {

    constructor(data) {
        if (typeof data !== "object") data = {};
        this.x = data.x ?? 0;
        this.y = data.y ?? 0;
        this.r = data.r ?? 10;
        this.fill = data.fill ?? Logice.Color.DARK;
        this.stroke = data.stroke ?? Logice.Color.LIGHT;
        this.lineWidth = data.lineWidth ?? 4;
        this.lineDash = (data.lineDash ?? []).slice();
        this.shape = data.shape ?? Logice.Shape.CIRCLE;
    }

    path(ctx, view = null) {
        if (!view) view = new Apper.Viewport();
        this.r = Math.abs(this.r);
        const pos = view.transform(this);
        ctx.beginPath();
        switch (this.shape) {
            case Logice.Shape.CIRCLE:
            default:
                ctx.ellipse(pos.x, pos.y, this.r * view.zoom, this.r * view.zoom, 0, 0, 2 * Math.PI);
                break;
            case Logice.Shape.SQUARE:
                ctx.rect(pos.x - this.r * view.zoom, pos.y - this.r * view.zoom, 2 * this.r * view.zoom, 2 * this.r * view.zoom);
                break;
            case Logice.Shape.DIAMOND:
                ctx.moveTo(pos.x, pos.y - 1.3 * this.r * view.zoom);
                ctx.lineTo(pos.x - 1.3 * this.r * view.zoom, pos.y);
                ctx.lineTo(pos.x, pos.y + 1.3 * this.r * view.zoom);
                ctx.lineTo(pos.x + 1.3 * this.r * view.zoom, pos.y);
                ctx.closePath();
                break;
        }
    }

    draw(ctx, hover, select, highlight, view = null) {
        if (!view) view = new Apper.Viewport();
        this.r = Math.abs(this.r);
        if (hover || select) {
            ctx.strokeStyle = select ? `${highlight}cc` : `${highlight}99`;
            ctx.lineWidth = this.lineWidth * view.zoom + 4;
            ctx.setLineDash([]);
            this.path(ctx, view);
            ctx.stroke();
        }
        ctx.fillStyle = this.fill;
        ctx.strokeStyle = this.stroke;
        ctx.lineWidth = this.lineWidth * view.zoom;
        ctx.setLineDash(this.lineDash.map(value => value * this.lineWidth * view.zoom));
        this.path(ctx, view);
        if (this.r > 0) ctx.fill();
        if (this.lineWidth > 0) ctx.stroke();
    }

    contains(ctx, pos, select, view = null, screen = false) {
        if (!view) view = new Apper.Viewport();
        if (!screen) pos = view.transform(pos);
        ctx.lineWidth = select ? this.lineWidth * view.zoom + 4 : Math.max(this.lineWidth * view.zoom + 4, 12);
        this.path(ctx, view);
        return ctx.isPointInPath(pos.x * view.scale, pos.y * view.scale) || ctx.isPointInStroke(pos.x * view.scale, pos.y * view.scale);
    }

    copy() {
        return new Logice.Vertex(this);
    }

    get margin() {
        return (this.shape === Logice.Shape.DIAMOND ? 1.3 : 1) * this.r + 0.5 * this.lineWidth;
    }

};


Logice.Edge = class {

    constructor(data) {
        if (typeof data !== "object") data = {};
        this.v1 = data.v1;
        this.v2 = data.v2 ?? data.v1;
        this.cp = data.cp == null ? null : data.cp instanceof Array ? new Apper.Vector2(data.cp[0], data.cp[1]) : new Apper.Vector2(data.cp);
        this.stroke = data.stroke ?? Logice.Color.DARK;
        this.lineWidth = data.lineWidth ?? 4;
        this.lineDash = (data.lineDash ?? []).slice();
    }

    path(ctx, view = null) {
        if (!view) view = new Apper.Viewport();
        ctx.beginPath();
        const v1 = view.transform(this.v1), v2 = view.transform(this.v2);
        if (this.cp == null) {
            ctx.moveTo(v1.x, v1.y);
            ctx.lineTo(v2.x, v2.y);
        } else if (!this.isLoop) {
            const cp = view.transform(this.cp);
            ctx.moveTo(v1.x, v1.y);
            ctx.quadraticCurveTo(cp.x, cp.y, v2.x, v2.y);
        } else {
            const cp = view.transform(this.cp);
            let radius = 0.5 * Math.hypot(v1.x - cp.x, v1.y - cp.y);
            ctx.ellipse(0.5 * (v1.x + cp.x), 0.5 * (v1.y + cp.y), radius, radius, 0, 0, 2 * Math.PI);
        }
    }

    draw(ctx, hover, select, highlight, view = null) {
        if (!view) view = new Apper.Viewport();
        if (hover || select) {
            ctx.strokeStyle = select ? `${highlight}cc` : `${highlight}99`;
            ctx.lineWidth = this.lineWidth * view.zoom + 4;
            ctx.setLineDash([]);
            this.path(ctx, view);
            ctx.stroke();
        }
        ctx.strokeStyle = this.stroke;
        ctx.lineWidth = this.lineWidth * view.zoom;
        ctx.setLineDash(this.lineDash.map(value => value * this.lineWidth * view.zoom));
        this.path(ctx, view);
        if (this.lineWidth > 0) ctx.stroke();
        if (select && this.cp !== null) {
            const v1 = view.transform(this.v1), v2 = view.transform(this.v2), cp = view.transform(this.cp);
            if (!this.isLoop) {
                ctx.strokeStyle = `${highlight}77`;
                ctx.lineWidth = 2;
                ctx.setLineDash([2, 5]);
                ctx.beginPath();
                ctx.moveTo(v1.x, v1.y);
                ctx.lineTo(cp.x, cp.y);
                ctx.moveTo(v2.x, v2.y);
                ctx.lineTo(cp.x, cp.y);
                ctx.stroke();
            }
            ctx.fillStyle = `${highlight}cc`;
            ctx.setLineDash([]);
            ctx.beginPath();
            ctx.moveTo(cp.x, cp.y + 8);
            ctx.lineTo(cp.x + 8, cp.y);
            ctx.lineTo(cp.x, cp.y - 8);
            ctx.lineTo(cp.x - 8, cp.y);
            ctx.closePath();
            ctx.fill();
        }
    }

    contains(ctx, pos, select, view = null, screen = false) {
        if (!view) view = new Apper.Viewport();
        if (!screen) pos = view.transform(pos);
        if (this.cp !== null) {
            const cp = view.transform(this.cp);
            ctx.beginPath();
            ctx.ellipse(cp.x, cp.y, 10, 10, 0, 0, 2 * Math.PI);
            if (ctx.isPointInPath(pos.x * view.scale, pos.y * view.scale)) return true;
        }
        ctx.lineWidth = select ? this.lineWidth * view.zoom + 4 : Math.max(this.lineWidth * view.zoom + 4, 12);
        this.path(ctx, view);
        return ctx.isPointInStroke(pos.x * view.scale, pos.y * view.scale);
    }

    copy() {
        return new Logice.Edge({ ...this, v1: this.v1.copy(), v2: this.isLoop ? undefined : this.v2.copy() });
    }

    get margin() {
        return 0.5 * this.lineWidth;
    }

    get isLoop() {
        return this.v1 === this.v2;
    }

};


window.addEventListener("load", event => {
    const vertedge = new Logice();
    document.body.appendChild(vertedge.element);
    vertedge.start();
});
